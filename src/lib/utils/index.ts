      ...
      const componentSlots = Object.entries(slots).reduce(
        (slotResult, [slotName, slot]) => {
          ...
          if (slotName.startsWith('bind:')) {
            try {
              const [, propInfo] = slotName.split(':', 2);

              const [propName, converter = 'obj'] = propInfo.split('.', 2);
              const propValue = extractTextFromVNode(slot());

              propStorage.attach(propName, propValue, converter as Converter);
            } catch (error) {
              console.error(error);
            }
